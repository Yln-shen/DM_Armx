# DmArm 封装层设计文档（v0.9 —— Joint 落地，MIT 优先）
...
| # | 内容 | 依据 / 证据 |
|---|---|---|
| 44 | **新增 `joint.py`**：`Joint` + `JointState`。薄形态（按 §四 类图）——**不拥有控制循环**、不 poll、不做寄存器 I/O。`set_mit()` 是「发一帧」不是「走到位」 | 用户决策：A（最小可跑）+ MIT 优先 |
| 45 | **§4.1 的 `JointState` 落地，但 `vbus` 恒为 `None`**。母线电压在寄存器 0x3C，而本层不做寄存器 I/O（写一次 ~150ms 且期间发不出帧，见 §六）。字段先留着；要它得等 `RegisterTool` 在**上电自检**阶段读一次 | §六硬性事实 3；§4.2 的 `vbus` 字段 |
| 46 | **`Joint` 顺手把 `limit` 注册到 `MotorBus`**，但同一个 ID 若已注册过**不同**档位则直接 `ValueError`。理由：同一个电机出现两个映射范围 = 静默档位混用，正是 D5 那个"力矩差 2.8 倍且不报错"的场景 | D5；`smoke_joint.py` [1] |
| 47 | **钳位分两层**：软限位（**关节侧**，`position_min/max`，本轮默认 `None` 不检查）→ 换算 → **PMAX 硬钳位（电机侧）**。顺序不能反：软限位是人为约定，PMAX 是电机侧的硬事实 | 用户决策：钳位 + **打警告**（不抛异常）—— 7 关节满载时一个关节越界把整条循环炸掉，比走到边界上更糟 |
| 48 | **MIT 的四个量超满量程会"静默钳位"，`Joint` 替它出声**。`float_to_uint` 钳位后**不报错**：`kp=999` 实际变成 500，`tau` 要 999 只给到 TMAX。臂会软趴趴地掉下来而你不知道为什么 | `smoke_joint.py` [5] |
| 49 | **★ 发现并钉死：MIT 的「零力矩」不是全零字节，是中点 2047。** `t_ff=0` 在 12 位区间 `[-TMAX,+TMAX]` 上映射到**中点**；`dq=0` 同理。**漏填 `t_ff` 让字节保持 0，发出去的是 `-TMAX` 满负力矩**，不是零 | `smoke_joint.py` [7]；本轮第一版测试就栽在这里（断言"全零"，红了） |
| 50 | **`enable()` 的时序内置**：发完使能帧**立刻补一帧零力矩**，把"已使能但没命令"的窗口压到最小。抄的是 `dm_bringup.py cmd_jog_mit` 的 `[1]` 步。分两步让调用方补，就有漏掉的机会 | `dm_bringup.py:694-698` |
| 51 | **故障态拒绝使能**：缓存里 `ERR` 非 0/1 → 拒绝并提示"ERR=13 是锁存的，要断电才能清"。⚠️ 已知代价：刚断过电而没重新 `poll()` 时会**误拒**（缓存是过期的）—— 保守方向，安全 | `smoke_joint.py` [8] |
| 52 | **`assert_healthy()`：先失能再抛异常。** 异常消息里必须写明「**失能 ≠ 停下**」—— 重力负载下电机松掉会直接掉下来，这跟"安全停车"是两回事 | 用户确认的语义；`smoke_joint.py` [9] |
| 53 | **三个预留桩**：`set_pos_vel` / `set_force_pos` / `switch_mode` 全部 `NotImplementedError`，且异常里写清**为什么**不能（要写 0x0A / 是 RegisterTool 的活）。不留"看起来能用其实是空壳"的方法 | §四 类图；本轮用户指定 |
| 54 | **`tools/smoke_joint.py` 55 项离线检查**，并用 **5 次定向突变**验证它**不是空的**（去掉 direction 翻号 / 去掉 PMAX 钳位 / 去掉零力矩帧 / 去掉满量程告警 / 去掉档位冲突拒绝 → 每次都变红） | ARCHITECTURE §六 的纪律：测试必须能被证伪 |
| 55 | **[11] 是静态安全边界检查**：用 `ast` 解析 `joint.py`，断言它**不 import** `serial` / `DM_CAN` / `dm_registers` / `numpy`，代码里**不出现** `set_zero_position` / `Serial(` / `save_motor_param`；再动态地把能发的帧全发一遍，逐帧确认**没有 0xFE（设零位）形状** | §六「全仓库只有一处会写 flash」；`set_zero_position` 全项目从未被调用 |

> **本轮**没有新的硬件实测** —— 它验的是软件语义（换算 / 钳位 / 时序 / 失败路径）。
> `joint.py` **至今没有跟真电机说过话**，这是下一步（真机 MIT 点动）要补的。
> 已实测的仍然只是**单电机、裸电机空载**（4310 与 4340P 各一只）。
...
### v0.9 → v0.10（2026-09-23 晚，**真机巡检 5 台** + 一次文档纠错的复盘）

**本轮是硬件轮，不动封装层代码。** 目的是把 5 台在架电机逐台核一遍，顺便把
`--save` 到底管不管用这件事**用一次断电定死**（之前文档和硬件对不上，见 #56）。

| # | 内容 | 依据 |
|---|---|---|
| 56 | **★ `--save` 是真的，已验证：`0x01` 写 750ms → 断 24V → 重上电 → 仍读 750ms。** 而且**同一次断电顺带证明了存档帧是"点对点"的**：`0x02`~`0x05` 全部仍是 0，没有被广播误伤。此前怀疑"`--save` 不生效"，是因为**当时拿回读值当证据**（见 #58） | `scan_bus.py` 多次；`DM_CAN.save_motor_param` 走 `0x7FF` + data 里带 ID，与 `refresh_frame` 同构 |
| 57 | **存档帧的 CAN ID 是 `0x7FF`（广播地址），但数据里带着目标 ID，所以只有目标应答。** 这是个很容易看错的组合 —— 光看 ID 会以为"存一次全存" | `DM_CAN.py:...save_motor_param`；实测 `0x02`~`0x05` 未被动 |
| 58 | **★★ `--save` 不改变回读值，所以「回读 ✓」不能证明它发生过。** 因为写入本来是先落 RAM、`--save` 只是把 RAM 抄进 flash。**`--save` 只有一个验法：跨一次断电再读。** 上表中「回读 10000 ✓」单独看是**无效证据** | 本轮 D1；此前文档与硬件矛盾就出在这里 |
| 59 | **★ 看门狗在失能状态下不计时 —— 真机确认。** `0x02` 写完 500ms 后静默 2s，读回 `ERR = 0x0`（**没有** ERR=13）。手册说"只在使能时生效"，这下是自己测出来的了 | 手册（`Reg 0x09` 注释）→ 本轮实测。注：这条**结论**早就在 `dm_registers.py` 的注释里，本轮只是从"书上说"变成"我测过" |
| 60 | **★ `0x01` 和 `0x03` 的温度/电压寄存器（`0x3C`/`0x3D`/`0x3E`）恒读 0，但它们的反馈帧里的温度是真实值。** 连续 3 次读都是 0。5 台里坏 2 台 ⇒ **不是个体故障，是这两台固件的寄存器行为**。结论：**温度必须从反馈帧取，不能指望寄存器** | 本轮逐台 dump；`MotorState.temp_mos/temp_rotor` 已从 `D[6]/D[7]` 取，设计无需改 |
| 61 | **反馈路由必须用 `D[0] & 0x0F`** —— 5 台电机的反馈帧 CAN ID 全是 `0x000`，靠 ID 根本分不出来。这条是旧结论，本轮 5 台并发时再次确认 | `dm_bus.poll()` 已按此实现；本轮实测复核 |
| 62 | **POS_VEL 固件的 PID 寄存器终于读到了**（`0x01`/`0x02` 两台一致）：`KP_ASR = 0.00384`、`KI_ASR = 0.002`、`KP_APR = 54`、`KI_APR = 0`。**只是记录，本轮没人用它们** —— POS_VEL 路径整个还是桩 | 本轮 `dm_registers dump` |
| 63 | **`0x0A`（CTRL_MODE）** 5 台全是 **1（MIT）**，`ERR` 全是 `0x0`（失能）。**注意 `0x0A` 是 RAM 不是 flash**，所以"现在是 MIT"不保证断电后还是 | 本轮逐台读 |
...
> **本轮之后台架实况**：`0x01` = 750ms（flash）；`0x02`~`0x05` = 0；5 台全 MIT、全失能。
> **下一步 D3（真机 MIT 点动）开始前，必须先过 `## 决定点`** —— 那是本项目第一次
> 不可逆的物理风险（撞限位），且 `Joint` 是薄类**不拥有循环**，循环得另写。

### v0.7 → v0.8（2026-09-23，`dm_frames` / `dm_bus` 脱开官方 SDK）

**先说清楚这一步去的是什么**：去的是「**封装层依赖厂商工具包**」，不是「全项目去 SDK」。
`dm_bringup.py` / `watchdog_test.py` 的**寄存器读写**仍然要走 SDK 的 `read_motor_param`，
`third_party/Python例程/` **不能删**。封装层不该依赖一个厂商库 —— 它的接口随版本变，
而我们的字节格式不该跟着变。

| # | 内容 | 依据 |
|---|---|---|
| 38 | **`dm_frames.py` 自持发送帧模板**：30 字节魔数从 `DM_CAN.MotorControl.send_data_frame` 抄成模块常量 `_TX_TEMPLATE`；`tx_template()` 每次返回**新的 `bytearray`**（返回同一个对象会让调用方互相污染） | 证据是 `smoke_dm_frames.py` [6] 拿 SDK 的 `controlMIT` 做**整帧 30 字节对拍** —— 抄错任何一个字节立刻红 |
| 39 | **`float_to_uint` / `float_to_uint8s` 自持**，与 SDK 逐字一致（钳位后**向零截断**，不是四舍五入）。`numpy` 随之从本层消失 | SDK `DM_CAN.py:565-591`。⚠️ 边界实测：`inf`/`-inf`/`NaN` 与 SDK 行为**完全相同**（NaN 两边都抛 `ValueError`，不会静默映射到负极限） |
| 40 | **六个构造函数去掉 `DM_CAN` 参数**：`MotorBus(sdk, port, ...)` → `MotorBus(port, ...)`。全仓库 **31 个调用点、6 个文件** | `grep -rn "(DM_CAN"` 已归零 |
| 41 | **`build_tx` 显式校验数据必须 8 字节**。numpy 换 `bytearray` 时有个**真实的坑**：切片赋值长度不等会把帧**悄悄改长**，后面所有字节错位且**不报错**；原来 numpy 会直接报错，这层保护不能丢 | 本次改动自己引入的风险，已挡住 |
| 42 | **测试里的 SDK 对照保留**，没有换成固化常量。理由：那是**唯一的外部真值** —— 换成"自己昨天的输出"会把当初抄错的一起固化，测试永远绿 | `smoke_dm_frames.py` 开头声明的原则 |
| 43 | **`pos_vel_frame` 补了直接对拍**（`smoke_dm_frames.py` [1b]）。原先 [1] 比的是「SDK 的 `float_to_uint8s`」vs「测试自己 `struct.pack` 的帧」，**两条路都不经过构造器** —— 改动 `float_to_uint8s` 或 CAN ID 的 `0x100` 偏移，[1] 全绿而电机收到乱码。已用两次变异测试验证新检查**不是空的** | `tools/smoke_dm_frames.py` [1b] |
...
| 28 | 阈值建议 200ms | 改为 **500ms**：主机侧控制循环 200~300Hz（D2），500ms ≈ 100~150 个周期余量，容得下调度抖动又远短于"人跑过去拔电源"。~~已 `--save` 写 flash 并断电重启验证持久生效~~ —— **这半句已作废**：那次写入被后来一次带 `--save` 的 `0x09=0` 覆盖，方法本身没错，是结果被后手改了。见下文「上线形态验证」的失效注记。**（建议值仍是 500ms；`0x01` 现在的 750ms 只是 2026-09-23 测持久性时临时挑的一个"区别于 500"的值，不是设计决定。）** | D4 取值建议 + 上线形态验证 |
