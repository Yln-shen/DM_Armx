...
在`damiao.py` 中 的 `DM_Motor_Type` 类中添加新型号，在 `limit_param` 数组中添加对应的限位参数。

### 添加枚举类型

在 `damiao.py` 中 的 `DM_Motor_Type` 类中，在 `Num_Of_Motor` 之前添加新型号：
...
在 `damiao.py` 中 的 `limit_param` 数组末尾，按与枚举完全相同的顺序添加一组参数：
