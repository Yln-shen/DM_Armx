...
在`DM_CAN.py` 中 的 `DM_Motor_Type` 类中添加新型号，在 `limit_param` 数组中添加对应的限位参数。

#### 添加枚举类型

在 `DM_CAN.py` 中 的 `DM_Motor_Type` 类中，在 `Num_Of_Motor` 之前添加新型号：
...
注意：新型号要加在已有型号之后，并且不要插在已有型号中间，否则已有型号的下标会改变，`limit_param` 会对应错。

#### 添加 limit_param

在 `DM_CAN.py` 中 的 `limit_param` 数组末尾，按与枚举完全相同的顺序添加一组参数：
